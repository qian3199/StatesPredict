...
def parse_args():
    ...
    parser.add_argument('--inference_files', type=int, default=0, help='推理时使用的文件数量，0 表示全部 (默认：0)')
    return parser.parse_args()

def save_model_path_config(output_dir, model_name):
    """保存模型路径配置"""
    config = {
        f'{model_name}_best_model': os.path.join(output_dir, 'best_model.pth'),
        f'{model_name}_last_model': os.path.join(output_dir, 'last_model.pth'),
        f'{model_name}_output_dir': output_dir
    }
    with open('latest_model_config.json', 'w') as f:
        import json
...
def run_training():
    ...
    logger.info(f"Data config: n_files={n_files}, hist_len={args.hist_len}, pred_len={args.pred_len}, step={args.step}")
    ...
    # 保存模型路径配置
    save_model_path_config(output_dir, model_name.upper())
    
    # 运行推理
    if args.inference_files > 0:
